...
function test() {
    fs.readFile('./code', function (error, file) {
        if (error) {
            ...
            // console.log('未加密：', data)
            var key = '123456';
            var encrypted = aesEncrypt(data, key);
            // var decrypted = aesDecrypt(encrypted, key);
            // console.log('第一层加密后：', encrypted)
            // 将加密后的文件输出
            fs.writeFile('./encrypted_code', encrypted, function (error) {
                if (error) {
            ...
            //         var encrypted_file = publicKey.encrypt(encrypted, 'base64');
            //         // console.log('第二层加密后：', encrypted_file)
            //     }
            // })
        }
    })
