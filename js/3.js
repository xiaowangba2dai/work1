...
// publicHashClient(string value, uint no)
function Client_3(hash1, no){
  contractInstance.methods.publicHashClient(hash1, no).send(
...
function main(){
  ...
  for (let i=0; i<Clients.length; i++){
    contractInstance.methods.Cget_no2('0x'+ Clients[i]).call().then(list=>{
      let no_array = list[0];
      let len = list[1];
      ...
      for(let j=0; j<len; j++){
        ...
        fs.readFile('./file/encrypt/encrypt_code'+no_array[j], function(error, data){
          if (error){
            console.log('读取encrypt_code'+no_array[j]+':失败')
          } else {
            let encrypt_file = data.toString();
            // 用取哈希
            let hash = sha256(encrypt_file);
            // 向区块链中写入哈希
            Client_3(hash, no_array[j]);
          }
        })
      }
